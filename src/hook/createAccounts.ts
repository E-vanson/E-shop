...
const url = "https://bctest.dayliff.com:7048/BC160/ODataV4/Company('KENYA')"
...
export async function addNewCustomer(customer: Customer): Promise<void> {
  const res = axios.post(`${url}/Customer_Card`, customer, config);
...
export async function getExistingCustomer(filterValue: string): Promise<boolean> {
  ...
  try {
  const res = await axios.get(`${url}/Customer_Card?${filter}`, config);
